...
function buildHermesConfigYaml({ provider, model }) {
...
export async function readHermesConfig(agentId) {
  ...
  const cmd = [
    'bash',
    '-lc',
    `if [ -f ${WORKSPACE_DIR}/${HERMES_CONFIG_FILENAME} ]; then cat ${WORKSPACE_DIR}/${HERMES_CONFIG_FILENAME}; fi`,
  ]
...
export async function readHermesEnv(agentId) {
  ...
  const cmd = [
    'bash',
    '-lc',
    `if [ -f ${WORKSPACE_DIR}/${HERMES_ENV_FILENAME} ]; then cat ${WORKSPACE_DIR}/${HERMES_ENV_FILENAME}; fi`,
  ]
  const result = await execInContainer(containerName, cmd)
  if ((result?.exitCode ?? 0) !== 0) {
    const stderrMsg = String(result?.stderr || '').trim()
